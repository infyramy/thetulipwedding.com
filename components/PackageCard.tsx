...
const PackageCard: React.FC<PackageCardProps> = ({ pkg }) => {
  const waLink = `https://wa.me/${WHATSAPP_NUMBER}?text=Assalamualaikum, saya nak tanya tentang pakej ${encodeURIComponent(pkg.name)}. Boleh bagi full details?`;
  ...
  return (
    <div className={`group relative flex flex-col h-full rounded-[2rem] overflow-hidden transition-all duration-500 hover:-translate-y-2 ${isVilla
      ...
      <div className="relative h-72 overflow-hidden z-10">
        <div className={`absolute inset-0 z-10 ${isVilla ? 'bg-gradient-to-t from-[#1A1A1A] to-transparent' : 'bg-gradient-to-t from-black/40 via-transparent to-transparent'}`}></div>
        <img
          src={pkg.image}
          alt={pkg.name}
        ...
        <div className="absolute top-5 left-5 z-20">
          <span className={`inline-block px-4 py-1.5 text-[10px] font-bold uppercase tracking-widest rounded-full backdrop-blur-md shadow-sm ${isVilla
        ...
        <div className="space-y-3 mb-10 flex-grow">
          ...
          {pkg.features.slice(0, 8).map((feature, idx) => (
            <div key={idx} className="flex items-start gap-3 group/item">
              <span className={`mt-0.5 w-5 h-5 rounded-full flex items-center justify-center shrink-0 transition-colors ${isVilla
        ...
        <div className="mt-auto">
          <a href={waLink} target="_blank" rel="noreferrer" className="block w-full group/btn">
            <Button
              ...
              className={`py-4 text-sm uppercase tracking-widest font-bold flex items-center justify-center gap-2 transition-all duration-300 ${isVilla
