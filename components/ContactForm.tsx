...
const ContactForm: React.FC = () => {
  ...
  const handleWhatsAppSubmit = () => {
    const text = `*Enquiry dari Website* 🌷

Assalamualaikum, saya nak tanya tentang pakej perkahwinan.
    
👤 *Nama:* ${formData.name}
📅 *Tarikh:* ${formData.date}
👥 *Tetamu:* ${formData.guests} pax
📦 *Minat:* ${formData.packageType}
    
Boleh tolong bagi quotation? Terima kasih! 😊`.trim();
    ...
    <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl shadow-gray-200/50 border border-white relative overflow-hidden">
      ...
      <div className="flex justify-between mb-12 relative px-4 max-w-xs mx-auto md:mx-0">
        ...
        {[1, 2, 3].map((i) => (
          <div
            key={i}
            className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold transition-all duration-500 border-4 ${step >= i ? 'bg-roseGold border-roseGold text-white scale-110 shadow-lg' : 'bg-white border-gray-200 text-gray-300'
              }`}
          >
            {i}
          </div>
